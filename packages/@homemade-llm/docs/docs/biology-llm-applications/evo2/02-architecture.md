...
真核ゲノムは長く複雑なので、論文では機能要素に注目した複数のサブセット——**genic（遺伝子）領域**、**プロモーター＋エクソン＋スプライス部位**、**mRNA**、**5 kb ウィンドウ**、**ncRNA**、**EPDnew（プロモーター DB）**、**オルガネラ**——を用意し、学習段階に応じて配合を変えています（Fig. 1d）。Extended Data Fig. 1a には、**界（kingdom）ごとの真核ゲノム数**・**サブセット別の総塩基対**・**augmented データの内訳** が示されており、OpenGenome2 が単なる配列の寄せ集めではなく、**機能を意識して設計・重み付けされたデータセット** であることが分かります。
...
Evo 2 はこの針を発見し、その値を予測できるか——というタスクです。結果、Evo 2 は **100 万トークンの文脈全体から針を正しく検索（recall）** できました。これは、文脈ウィンドウ全体の情報を実際に活用できていることの確認になります（retrieval スコア 0.8 以上で有意、$P < 0.001$）。

:::note[「本当に検索できている」の統計的な裏づけ（Extended Data Fig. 1e）]

この有意性は、**針の配列をランダムにシャッフルして帰無分布（null distribution）を作る** ことで検証されています（categorical Jacobian 解析にもとづく $N = 1040$ 個のスコア）。針が無意味になればスコアは下がるはず——その分布と比べ、スコア 0.8 を閾値にすると「検索できていない」という帰無仮説を $P < 0.001$ で棄却できます。「たまたま高いスコアが出ただけ」ではないことを、統計的に示しているわけです。
