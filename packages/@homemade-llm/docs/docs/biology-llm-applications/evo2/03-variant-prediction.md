...
## 8. さらに踏み込んだ評価（Extended Data）

本編で触れた結果の背後には、論文の Extended Data に豊富な追加解析があります。代表的なものを拾います。

### 翻訳のコドンランプと tAI（Extended Data Fig. 3d,e）

進化的制約のさらなる証拠が **翻訳のコドンランプ（codon ramp）** です。遺伝子の **最初の方のコドンには、ゆっくり翻訳される（tAI が低い）コドンが使われやすい**——リボソームの渋滞を防ぐためと考えられる既知の現象です。**tRNA 適応指数（tRNA-adaptation index, tAI）** で解析すると、Evo 2 は同義置換であっても **tAI を下げる方向の変異を「ありにくい」と予測** しました（4 種で確認）。アミノ酸を変えない変異の「効率」まで捉えている、ということです。

### ClinVar の層別解析（Extended Data Fig. 4）

先ほどの ClinVar 評価を、**保存度（PhyloP）別** や **スプライス部位への近さ別** に層別しても、Evo 2 は頑健でした。とくに **保存度が低い領域** や **スプライス部位から遠い領域** でも教師なしモデルとして競争力を保ちます（SpliceAI/Pangolin のスコアが低い変異だけに絞っても傾向は維持）。

### BRCA1 の最適な層とプーリング（Extended Data Fig. 5d,e）

教師あり分類器（前節）では、**どの層の埋め込みを、変異まわりのどの窓幅で集約（pooling）するか** を系統的に探索しています。Evo 2 40B の **block 20 付近** の埋め込みが最も情報量が多く、5-fold 交差検証で最適なリッジ回帰の正則化パラメータを選びました。「大規模 LM の各層が異なる特徴を捉える」性質を、変異分類に活かした形です。

### BRCA2・制御領域・mRNA 分解（Extended Data Fig. 3g,i / 5c,f）

- **BRCA2**：coding/noncoding/aggregate のいずれでも高性能（GPN-MSA 超え、CADD に次ぐ）。
- **DART-eval**：cCRE 判別（Task 1）・TF モチーフ同定（Task 2）で他ベースラインを上回り、変異効果（Task 5, caQTL/dsQTL）では埋め込みが非コード変異効果の予測に有用。
- **mRNA 分解**：Evo 2 の尤度は、ヒト mRNA の実験的な **分解速度（decay rate）と負に相関**（長さ補正後）。
- **遺伝子必須性**：**DepMap** のヒト遺伝子必須性分類でも、保存度ベースラインや他の言語モデルと比較されています。
