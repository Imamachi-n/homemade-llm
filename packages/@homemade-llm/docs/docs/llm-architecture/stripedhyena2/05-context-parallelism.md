...
p2p の代表例 **ring attention** の動きを、もう少し細かく見てみましょう。各ランクは query・key・value をそれぞれ $[D,\, L/N_{cp}]$ で保持します。
...
4. $N_{cp}$ ステップ後、各ランクの query は **すべての key/value を見終え**、最終的な attention 結果の shard を保持する。
...
a2a 畳み込みについても、モデルとシーケンスが大きいほど **通信レイテンシがボトルネック** になります（1回のメッセージが巨大なため）。論文はこれを緩和する **channel-pipelined a2a** も提案しています。入力を $N_{pipe}$ 個のセグメントに分割し、**CUDA ストリームで非同期に a2a を回しながら計算とオーバーラップ** させることで、通信待ちの時間を計算で隠します。系列長方向ではなく **チャネル方向にパイプライン** する点が特徴です。
...
DFT の定義から FFT の分割統治、バタフライ（DiF/DiT）、Radix-$N$、そして分散 p2p FFT が**追加通信なしで成立する理由**まで——FFT 畳み込みの数学的な全体像は、付録 [06. FFT 畳み込みの数学と分散実装](./06-fft-convolutions.md) に独立してまとめました。数式でじっくり追いたい方はそちらへ。
