...
論文は再現性のため、学習の実務的な設定も詳しく開示しています。

**インフラと精度**

- 学習はオープンソースの **Savanna**（設定ファイルは凍結コミットで公開）。
- 混合精度学習で、**dense 層（SwiGLU・射影）と正規化層は FP8**。
- 7B のバッチサイズは **critical batch size estimation**（McCandlish et al.）で決定。

**並列化の構成（コンテキスト並列を含む）**

StripedHyena 2 は、Tensor Parallel・Sequence Parallel・[Context Parallel](./05-context-parallelism.md) を、系列長に応じて組み合わせます。

| 設定 | 7B（256 H100） | 40B（2,048 H100） |
| --- | --- | --- |
| Tensor Parallel | 2 → 32 | 8 → 64 |
| Context Parallel | 1 → 2 | 1 → 2 |
| Global Batch Size | 4M トークン | 8M トークン |

短い文脈では Tensor Parallel 中心、長い文脈になるほど Context Parallel を効かせる、という切り替えです（数値は文脈長 16K → 1M で増加）。

**グループ化の ablation**

前述のフィルタ・グループ化について、論文は品質への影響を検証しています。

- group size **1（共有なし）と 16 で収束に差なし**。
- **64 を超える**と小さな劣化（小さいモデルで顕著）。
- グループ数が少ないと、Hyena-MR の指数減衰正則化（チャネル横断で初期化）の **粒度が粗くなる** ため。

実用上は、品質を保ちつつテンソルコア効率を上げられる範囲で group size を選びます。**「ほぼ無料で速くなる」工夫が、実験でも裏づけられている** わけです。

:::tip[早期実験：FFN を畳み込みで置き換える]

論文は初期設計で、各 Hyena/MHA 演算子の後の **FFN（MLP・SwiGLU）を Hyena-SE で置き換えた** 変種も試しています。スループットがわずかに下がる代わりに収束が改善する傾向があり、Hyena-SE が MHA や状態空間モデルより高速なことと合わせ、「**将来のマルチハイブリッドは MLP と Hyena-SE の比率も最適化の対象になる**」と展望を述べています。
